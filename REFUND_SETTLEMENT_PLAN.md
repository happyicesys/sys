...
**Status simplified to open/closed (2026-07-06):** the settlement now has only **open** and **closed** — the `exported` and `done` states were dropped. Exporting the CIMB file no longer changes status (it just records the file); marking rows done no longer flips the settlement to "done" (individual tickets still complete). The Close ⇄ Undo-close toggle and the Export CIMB button are always present; per-row **Mark done** appears once the settlement is Closed, **Remove** only while Open. Migration `2026_07_06_130000` folds any legacy `exported`/`done` rows into `closed`; a per-group `payout_groups` CIMB account is managed via the new **Admin → Operator Group** CRUD.
