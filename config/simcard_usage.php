...
return [
    ...
    | by carrying the key in telcos.usage_provider (the "Usage API" select on
    | the SimCard Package form); telcos without one are skipped by
    | simcards:sync-usage, so no master switch is needed — an instance with no
    | mapped telco simply no-ops. 'endpoint' is the default query link; a
    | package can override it with telcos.usage_endpoint ("API query link" on
    | the same form) when the same provider serves it from another URL.
    */
    'providers' => [

        'voiceping' => [
            'class' => VoicePingUsageProvider::class,
            'label' => 'VoicePing sim-info',
