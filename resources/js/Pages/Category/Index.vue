                ...
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(category, categoryIndex) in categories.data" :key="category.id" class="divide-x divide-gray-200">
                      ...
                      <TableData :currentIndex="categoryIndex" :totalLength="categories.length" inputClass="text-left">
                        {{ category.name }}
                      </TableData>
                      <TableData :currentIndex="categoryIndex" :totalLength="categories.length" inputClass="text-left whitespace-pre-wrap">
                        {{ category.desc }}
                      </TableData>
                      <TableData :currentIndex="categoryIndex" :totalLength="categories.length" inputClass="text-left">
                        {{ category.category_group_id ? category.category_group_id.name : null }}
                      </TableData>
                      <TableData :currentIndex="categoryIndex" :totalLength="categories.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          <Button
                            type="button" class="bg-gray-300 hover:bg-gray-400 px-3 py-2 text-xs text-gray-800 flex space-x-1"
                            @click="onEditClicked(category)"
                          >
                          ...
                          <Button
                            ...
                            @click="onDeleteClicked(category)"
                          >
...
function onDeleteClicked(category) {
  const approval = confirm('Are you sure to delete ' + category.name + '?');
  if (!approval) {
      return;
  }
