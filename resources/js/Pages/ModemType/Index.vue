        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex flex-col space-y-2">
              <p class="text-sm text-gray-700 leading-5 flex space-x-1">
                  <span>Showing</span>
                  <span class="font-medium">{{ modemTypes.meta.from ?? 0 }}</span>
                  <span>to</span>
                  <span class="font-medium">{{ modemTypes.meta.to ?? 0 }}</span>
                  <span>of</span>
                  <span class="font-medium">{{ modemTypes.meta.total }}</span>
                  <span>results</span>
              </p>
              <MultiSelect
      ...
      <div class="mt-6 flex flex-col">
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  <tr class="divide-x divide-gray-200">
                    ...
                    <TableHeadSort modelName="alias" :sortKey="filters.sortKey" :sortBy="filters.sortBy" @sort-table="sortTable('alias')">
                      Alias
                    </TableHeadSort>
                    <TableHead>
                    </TableHead>
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(modemType, modemTypeIndex) in modemTypes.data" :key="modemType.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      ...
                      <TableData :currentIndex="modemTypeIndex" :totalLength="modemTypes.length" inputClass="text-left">
                        {{ modemType.name }}
                      </TableData>
                      <TableData :currentIndex="modemTypeIndex" :totalLength="modemTypes.length" inputClass="text-left">
                        {{ modemType.alias }}
                      ...
                      <TableData :currentIndex="modemTypeIndex" :totalLength="modemTypes.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          ...
                          <Button
                            ...
                            :class="[modemType.modemUnits && modemType.modemUnits.length > 0 ? 'opacity-50 cursor-not-allowed' : '']"
                            @click="onDeleteClicked(modemType)"
                            :disabled="modemType.modemUnits && modemType.modemUnits.length > 0"
                          >
                            ...
                            <span v-if="modemType.vends && modemType.vends.length > 0">
                              (Binded)
                            </span>
                          </Button>
                        </div>
                ...
                <tr v-if="!modemTypes.data.length">
            ...
            <Paginator v-if="modemTypes.data.length" :links="modemTypes.links" :meta="modemTypes.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/ModemType/Form.vue';
...
function onCreateClicked() {
  type.value = 'create'
  modemType.value = null
  showModal.value = true
}

function onDeleteClicked(modemType) {
  const approval = confirm('Are you sure to delete ' + modemType.name + '?');
  if (!approval) {
      return;
  }
...
function onEditClicked(telcoValue) {
  type.value = 'update'
  modemType.value = telcoValue
  showModal.value = true
}

function onSearchFilterUpdated() {
  router.get('/modem-types', {
      ...filters.value,
