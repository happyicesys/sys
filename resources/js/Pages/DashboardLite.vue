<template>
    ...
    <Head title="Performance (Lite)" />

    <BreezeAuthenticatedLayout>
        <template #header>
            <div class="flex flex-col space-y-1">
                <div class="flex space-x-2 items-center">
                    Performance Dashboard (Lite)
                </div>
            </div>
        </template>
        ...
        <div class="p-3">
            <div class="max-w-7xl mx-auto sm:px-3 lg:px-2">
                <!--
                  The twin of Dashboard.vue's amber banner, saying the opposite.
                  This page reads vend_product_records, whose grain is date x
                  machine x PRODUCT, so VendProductRecord's
                  ProductAccessProductColumnScope narrows every figure below to
                  the viewer's own "Access Product(s)" allow-list. Shown only to a
                  restricted viewer - an unrestricted admin IS seeing everything
                  here, and the note would be a lie.
                -->
                <div
                    v-if="productRestricted"
                    class="mb-3 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800"
                >
                    <span class="font-semibold">Your products only.</span>
                    Every figure below covers just the products you have access to, not the whole machine.
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
                        ...
                        <div>
                            ...
                            <MultiSelect
                                v-model="filters.productCategories"
                        ...
                        <div>
                            ...
                            <MultiSelect
                                v-model="filters.productCategoryGroups"
                                :options="productCategoryGroupOptions"
                                trackBy="id"
                                valueProp="id"
                                label="name"
                                placeholder="Select"
...
import OperatorFilter from '@/Components/OperatorFilter.vue';
    ...
    const props = defineProps({
        ...
        // Lite-only: the product filter row. vend_product_records carries the
        // product dimension, so unlike Performance this page can filter on it.
        productOptions: Array,
        productCategoryOptions: Array,
        productCategoryGroupOptions: Array,
    });
    const filters = ref({
        codes: '',
        customer: '',
    ...
    function buildDashboardQueryParams(overrides = {}) {
        ...
        return {
            ...
            // Names match VendProductRecord::scopeFilterIndex, which takes arrays.
            product_id: filters.value.products.map((product) => { return product.id }),
            product_category_id: filters.value.productCategories.map((category) => { return category.id }),
            product_category_group_id: filters.value.productCategoryGroups.map((group) => { return group.id }),
            // The `...filters.value` spread above puts the raw MultiSelect OBJECTS
            // in the query string, so a picked product would ride along as
            // products[0][id]=451&products[0][name]=U-15%20-%20Wall's... — ~90 chars
            // each, and a product owner with 50 SKUs would push a multi-KB URL.
            // Collapse them to bare ids, exactly as vendModels/vendPrefixes above do.
            products: filters.value.products.map((product) => { return product.id }),
            productCategories: filters.value.productCategories.map((category) => { return category.id }),
            productCategoryGroups: filters.value.productCategoryGroups.map((group) => { return group.id }),
    ...
    function onSearchFilterUpdated() {
        router.visit(
            route('dashboard.performance-lite', buildDashboardQueryParams()),{
    ...
    function resetFilters() {
        router.get('/dashboard/performance-lite', {}, {
            preserveState: false,
            preserveScroll: true,
        })
    }
