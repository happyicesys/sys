<template>
  ...
  <Head title="Machine Sticker" />

  <BreezeAuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        Data Management (Machine Sticker)
      </h2>
    </template>
        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex space-x-3 items-center">
            <div class="text-sm text-gray-700">
                Showing
                <span class="font-medium">{{ vendStickers.meta.from ?? 0 }}</span>
                to
                <span class="font-medium">{{ vendStickers.meta.to ?? 0 }}</span>
                of
                <span class="font-medium">{{ vendStickers.meta.total }}</span>
                results
            </div>
            <MultiSelect
      ...
      <div class="mt-6 flex flex-col">
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(vendSticker, vendStickerIndex) in vendStickers.data" :key="vendSticker.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      <TableData :currentIndex="vendStickerIndex" :totalLength="vendStickers.length" inputClass="text-center">
                        {{ vendStickers.meta.from + vendStickerIndex }}
                      </TableData>
                      ...
                      <TableData :currentIndex="vendStickerIndex" :totalLength="vendStickers.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          <Button v-if="permissions.update"
                            ...
                            @click="onEditClicked(vendSticker)"
                          >
                          ...
                          <Button v-if="permissions.delete"
                            ...
                            @click="onDeleteClicked(vendSticker)"
                          >
                ...
                <tr v-if="!vendStickers.data.length">
            ...
            <Paginator v-if="vendStickers.data.length" :links="vendStickers.links" :meta="vendStickers.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/VendSticker/Form.vue';
...
function onCreateClicked() {
  type.value = 'create'
  vendSticker.value = null
  showModal.value = true
}

function onDeleteClicked(vendSticker) {
  const approval = confirm('Are you sure to delete ' + vendSticker.name + '?');
  if (!approval) {
      return;
  }
  router.delete('/machine-stickers/' + vendSticker.id, {
    onSuccess: () => {
      toast.success("Machine sticker deleted successfully", { timeout: 3000 })
    },
    onError: () => {
      toast.error("Failed to delete machine sticker", { timeout: 3000 })
    }
  })
}

function onEditClicked(vendStickerValue) {
  type.value = 'update'
  vendSticker.value = vendStickerValue
  showModal.value = true
}

function onSearchFilterUpdated() {
  router.get('/machine-stickers', {
      ...filters.value,
