...
const statusLabels = { open: 'Open', closed: 'Closed' };
...
function applyFilters() {
    router.get('/site-settlements', { ...filters.value }, { preserveState: true, replace: true });
}
...
<template>
<Head title="Site Settlement" />
<BreezeAuthenticatedLayout>
    <template #header>
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Site Settlement</h2>
    </template>
    ...
    <div class="m-2 sm:mx-5 sm:my-3 px-1 sm:px-2 lg:px-3">
        ...
        <div class="bg-white rounded-md border p-3 mb-3 grid grid-cols-1 md:grid-cols-6 gap-2 items-start">
            <input v-model="filters.search" placeholder="CST reference" class="border rounded-md px-3 py-2 text-sm md:col-span-2" @keyup.enter="applyFilters" />
        ...
        <div class="bg-white rounded-md border overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    <tr v-for="s in settlements.data" :key="s.id" class="border-t hover:bg-gray-50 cursor-pointer" @click="router.visit('/site-settlements/' + s.id)">
                        ...
                        <td class="px-4 py-3 text-center whitespace-nowrap">{{ s.done_count }} / {{ s.row_count }}</td>
                    </tr>
                    <tr v-if="!settlements.data.length"><td colspan="7" class="px-4 py-8 text-center text-gray-400">No settlements yet. Push locked-unpaid rows from the Site Summary page.</td></tr>
                </tbody>
            </table>
        </div>
