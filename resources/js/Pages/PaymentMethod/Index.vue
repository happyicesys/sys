        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex flex-col space-y-2">
              <p class="text-sm text-gray-700 leading-5 flex space-x-1">
                  <span>Showing</span>
                  <span class="font-medium">{{ paymentMethods.meta.from ?? 0 }}</span>
                  <span>to</span>
                  <span class="font-medium">{{ paymentMethods.meta.to ?? 0 }}</span>
                  <span>of</span>
                  <span class="font-medium">{{ paymentMethods.meta.total }}</span>
                  <span>results</span>
              </p>
              <MultiSelect
       ...
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(paymentMethod, paymentMethodIndex) in paymentMethods.data" :key="paymentMethod.id" class="divide-x divide-gray-200">
                      ...
                      <TableData :currentIndex="paymentMethodIndex" :totalLength="paymentMethods.length" inputClass="text-left">
                        {{ paymentMethod.name }}
                      </TableData>
                      <TableData :currentIndex="paymentMethodIndex" :totalLength="paymentMethods.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          <Button
                            ...
                            @click="onEditClicked(paymentMethod)"
                          >
                          ...
                          <Button
                            ...
                            @click="onDeleteClicked(paymentMethod)"
                          >
                ...
                <tr v-if="!paymentMethods.data.length">
            ...
            <Paginator v-if="paymentMethods.data.length" :links="paymentMethods.links" :meta="paymentMethods.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/PaymentMethod/Form.vue';
...
function onDeleteClicked(paymentMethod) {
  const approval = confirm('Are you sure to delete ' + paymentMethod.name + '?');
  if (!approval) {
      return;
  }
...
function onEditClicked(paymentMethodValue) {
  type.value = 'update'
  paymentMethod.value = paymentMethodValue
  showModal.value = true
}

function onSearchFilterUpdated() {
  Inertia.get('/payment-methods', {
      ...filters.value,
