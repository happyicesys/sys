        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex flex-col space-y-2">
              <p class="text-sm text-gray-700 leading-5 flex space-x-1">
                  <span>Showing</span>
                  <span class="font-medium">{{ zones.meta.from ?? 0 }}</span>
                  <span>to</span>
                  <span class="font-medium">{{ zones.meta.to ?? 0 }}</span>
                  <span>of</span>
                  <span class="font-medium">{{ zones.meta.total }}</span>
                  <span>results</span>
              </p>
              <MultiSelect
      ...
      <div class="mt-6 flex flex-col">
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(zone, zoneIndex) in zones.data" :key="zone.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      <TableData :currentIndex="zoneIndex" :totalLength="zones.length" inputClass="text-center">
                        {{ zones.meta.from + zoneIndex }}
                      </TableData>
                      <TableData :currentIndex="zoneIndex" :totalLength="zones.length" inputClass="text-left">
                        {{ zone.name }}
                      </TableData>
                      <TableData :currentIndex="zoneIndex" :totalLength="zones.length" inputClass="text-center">
            ...
            <Paginator v-if="zones.data.length" :links="zones.links" :meta="zones.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/Zone/Form.vue';
...
function onDeleteClicked(zone) {
  const approval = confirm('Are you sure to delete ' + zone.name + '?');
  if (!approval) {
      return;
  }
