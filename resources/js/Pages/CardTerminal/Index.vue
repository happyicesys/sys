        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex space-x-3 items-center">
            <div class="text-sm text-gray-700">
                Showing
                <span class="font-medium">{{ cardTerminals.meta.from ?? 0 }}</span>
                to
                <span class="font-medium">{{ cardTerminals.meta.to ?? 0 }}</span>
                of
                <span class="font-medium">{{ cardTerminals.meta.total }}</span>
                results
            </div>
            <MultiSelect
      ...
      <div class="mt-6 flex flex-col">
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(cardTerminal, cardTerminalIndex) in cardTerminals.data" :key="cardTerminal.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      <TableData :currentIndex="cardTerminalIndex" :totalLength="cardTerminals.length" inputClass="text-center">
                        {{ cardTerminals.meta.from + cardTerminalIndex }}
                      </TableData>
                      ...
                      <TableData :currentIndex="cardTerminalIndex" :totalLength="cardTerminals.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          <Button
                            ...
                            @click="onEditClicked(cardTerminal)"
                          >
                          ...
                          <Button
                            ...
                            @click="onDeleteClicked(cardTerminal)"
                          >
                ...
                <tr v-if="!cardTerminals.data.length">
            ...
            <Paginator v-if="cardTerminals.data.length" :links="cardTerminals.links" :meta="cardTerminals.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/CardTerminal/Form.vue';
...
function onCreateClicked() {
  type.value = 'create'
  cardTerminal.value = null
  showModal.value = true
}

function onDeleteClicked(cardTerminal) {
  const approval = confirm('Are you sure to delete ' + cardTerminal.name + '?');
  if (!approval) {
      return;
  }
...
function onEditClicked(cardTerminalValue) {
  type.value = 'update'
  cardTerminal.value = cardTerminalValue
  showModal.value = true
}

function onSearchFilterUpdated() {
  router.get('/card-terminals', {
      ...filters.value,
