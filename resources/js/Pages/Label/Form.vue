...
function submit() {
  ...
  if(props.type === 'create') {
    ...
    .post('/tags/create', {
      onSuccess: () => {
        toast.success(`${props.modelName} label created successfully`, { timeout: 3000 })
        emit('modalClose')
      },
      onError: () => {
        toast.error(`Failed to create ${props.modelName.toLowerCase()} label`, { timeout: 3000 })
      },
      preserveState: true,
      replace: true,
    })
  ...
  if(props.type === 'update') {
    form.value
      ...
      onSuccess: () => {
        toast.success(`${props.modelName} label updated successfully`, { timeout: 3000 })
        emit('modalClose')
      },
      onError: () => {
        toast.error(`Failed to update ${props.modelName.toLowerCase()} label`, { timeout: 3000 })
      },
      preserveState: true,
      replace: true,
    })
