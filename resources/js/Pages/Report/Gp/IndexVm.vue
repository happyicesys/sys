                ...
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(vend, vendIndex) in vends.data" :key="vend.id" class="divide-x divide-gray-200">
                      ...
                      <TableData :currentIndex="vendIndex" :totalLength="vends.length" inputClass="text-right">
                        {{ vend.last_two_month_gross_profit_margin ?? 0 }}
                      </TableData>
                    </tr>
...
function onExportExcelClicked() {
    ...
    axios({
        ...
        fileDownload(response.data, 'UnitCostByVm_' + moment().format('YYMMDDhhmmss') +'.xlsx')
    }).catch(error => {
        console.log(error)
    }).finally(() => {
        loading.value = false
