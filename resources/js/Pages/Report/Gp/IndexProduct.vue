            ...
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(product, vendIndex) in products.data" :key="product.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
...
function onExportExcelClicked() {
    ...
    axios({
        ...
        fileDownload(response.data, 'UnitCostByProduct_' + moment().format('YYMMDDhhmmss') +'.xlsx')
    }).catch(error => {
        console.log(error)
    }).finally(() => {
        loading.value = false
