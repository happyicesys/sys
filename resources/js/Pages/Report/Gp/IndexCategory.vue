            ...
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  <tr class="divide-x divide-gray-200">
                    ...
                    <TableHeadSort colspan="2" modelName="name" :sortKey="filters.sortKey" :sortBy="filters.sortBy" @sort-table="sortTable('name')">
                      Name
                    </TableHeadSort>
                    <TableHead colspan="4">
                      This Month
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(category, vendIndex) in categories.data" :key="category.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-center">
                        {{ categories.meta.from + vendIndex }}
                      </TableData>
                      <TableData colspan="2" :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-left">
                        {{ category.name }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.this_month_count.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.this_month_revenue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.this_month_gross_profit.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.this_month_gross_profit_margin ?? 0 }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_month_count.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_month_revenue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_month_gross_profit.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_month_gross_profit_margin ?? 0 }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_two_month_count.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_two_month_revenue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
                        {{ category.last_two_month_gross_profit.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}
                      </TableData>
                      <TableData :currentIndex="vendIndex" :totalLength="categories.length" inputClass="text-right">
...
onMounted(() => {
  ...
  categoryOptions.value = props.categoryOptions.data.map((data) => {return {id: data.id, name: data.name}})
...
function onSearchFilterUpdated() {
  router.get('/reports/gp/category', {
      ...filters.value,
...
function onExportExcelClicked() {
    ...
    axios({
        ...
        fileDownload(response.data, 'UnitCostByCategory_' + moment().format('YYMMDDhhmmss') +'.xlsx')
    }).catch(error => {
        console.log(error)
    }).finally(() => {
        loading.value = false
