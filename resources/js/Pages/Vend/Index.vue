        ...
        <div class="p-4 sm:px-6 lg:px-8">
            <div class="sm:flex sm:items-center">
                <section class="sm:flex sm:space-x-5 space-y-2 sm:space-y-0">
                    ...
                    <Button type="button" @click.prevent="onVendChannelErrorLogEmailClicked()">
                        Send Vending Machine Channel Error Log Mail
                    </Button>
                    <!-- <Select v-model="searchFilters.vends" :options="vends.data">
                        Vending Machine
                    </Select> -->
                </section>
            </div>
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="(vend, vendIndex) in vends.data" :key="vend.id"
                                        ...
                                        <td :class="[vendIndex !== vends.length - 1 ? 'border-b border-gray-200' : '', 'whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-blue-600 sm:pl-6 lg:pl-8']"
                                            ...
                                            <span class="hover:cursor-pointer" @click="onVendTempClicked(vend.id)">
                                                {{ vend.temp }}
                                            </span>
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800"
                                                v-if="vend.is_temp_error">
                                                Abnormal</span>
                                        </td>
                                        <td :class="[vendIndex !== vends.length - 1 ? 'border-b border-gray-200' : '', 'whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-600 sm:pl-6 lg:pl-8']"
                                            class="text-center">
