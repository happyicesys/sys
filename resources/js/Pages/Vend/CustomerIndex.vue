					...
					<TableData :currentIndex="vendIndex" :totalLength="vends.length" inputClass="text-center" v-if="!roles.includes('operator_driver')">
						<div class="flex flex-col space-y-1">
							...
							<!--
								...
								modem_type_alias). The modem unit's online state and
								last-seen live on the Machine Status column's
								"Remote Modem" badge, not here (Brian, 2026-09-12).

								Always rendered so the Payment Device column lines
								up across rows. Colour is kept in sync with the
								MQTT badge in the Machine Status column: green when
								the vend is MQTT-active, gray otherwise — never red,
								same as MQTT. Machines without MQTT fall back to the
								HTTP online flag.
							-->
							<div
									class="inline-flex justify-center items-center rounded px-1.5 py-0.5 text-xs font-medium border min-w-full"
									:class="[
										(vend.is_active || vend.is_testing)
											? ((vend.is_mqtt ? vend.is_mqtt_active : vend.is_online) ? 'bg-green-200' : 'bg-gray-200')
											: 'bg-gray-200 text-gray-400'
									]"
							>
									<div class="flex flex-col">
