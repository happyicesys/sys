<template>
  ...
  <Head title="Cashless Provider" />

  <BreezeAuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        Data Management (Cashless Provider)
      </h2>
    </template>
        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex flex-col space-y-2">
              <p class="text-sm text-gray-700 leading-5 flex space-x-1">
                  <span>Showing</span>
                  <span class="font-medium">{{ cashlessProviders.meta.from ?? 0 }}</span>
                  <span>to</span>
                  <span class="font-medium">{{ cashlessProviders.meta.to ?? 0 }}</span>
                  <span>of</span>
                  <span class="font-medium">{{ cashlessProviders.meta.total }}</span>
                  <span>results</span>
              </p>
              <MultiSelect
       ...
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(cashlessProvider, cashlessProviderIndex) in cashlessProviders.data" :key="cashlessProvider.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      ...
                      <TableData :currentIndex="cashlessProviderIndex" :totalLength="cashlessProviders.length" inputClass="text-left">
                        {{ cashlessProvider.name }}
                      </TableData>
                      <TableData :currentIndex="cashlessProviderIndex" :totalLength="cashlessProviders.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          <Button
                            type="button" class="bg-gray-300 hover:bg-gray-400 px-3 py-2 text-xs text-gray-800 flex space-x-1"
                            @click="onEditClicked(cashlessProvider)"
                          >
                          ...
                          <Button
                            ...
                            @click="onDeleteClicked(cashlessProvider)"
                          >
                ...
                <tr v-if="!cashlessProviders.data.length">
            ...
            <Paginator v-if="cashlessProviders.data.length" :links="cashlessProviders.links" :meta="cashlessProviders.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/CashlessProvider/Form.vue';
...
function onCreateClicked() {
  type.value = 'create'
  cashlessProvider.value = null
  showModal.value = true
}

function onDeleteClicked(cashlessProvider) {
  const approval = confirm('Are you sure to delete ' + cashlessProvider.name + '?');
  if (!approval) {
      return;
  }
...
function onEditClicked(cashlessProviderValue) {
  type.value = 'update'
  cashlessProvider.value = cashlessProviderValue
  showModal.value = true
}
