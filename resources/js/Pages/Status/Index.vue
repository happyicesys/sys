        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex flex-col space-y-2">
              <p class="text-sm text-gray-700 leading-5 flex space-x-1">
                  <span>Showing</span>
                  <span class="font-medium">{{ statuses.meta.from ?? 0 }}</span>
                  <span>to</span>
                  <span class="font-medium">{{ statuses.meta.to ?? 0 }}</span>
                  <span>of</span>
                  <span class="font-medium">{{ statuses.meta.total }}</span>
                  <span>results</span>
              </p>
              <MultiSelect
      ...
      <div class="mt-6 flex flex-col">
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
                ...
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(status, statusIndex) in statuses.data" :key="status.id" class="divide-x divide-gray-200">
                      ...
                      <TableData :currentIndex="statusIndex" :totalLength="statuses.length" inputClass="text-left">
                        {{ status.name }}
                      </TableData>
                      <TableData :currentIndex="statusIndex" :totalLength="statuses.length" inputClass="text-center">
            ...
            <Paginator v-if="statuses.data.length" :links="statuses.links" :meta="statuses.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
...
import Form from '@/Pages/Status/Form.vue';
...
function onDeleteClicked(status) {
  const approval = confirm('Are you sure to delete ' + status.name + '?');
  if (!approval) {
      return;
  }
  Inertia.delete('/statuses/' + status.id)
}
