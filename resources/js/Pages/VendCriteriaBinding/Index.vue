<template>
  ...
  <Head title="Location Type" />

  <BreezeAuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        Vend & Criteria Bindings
      </h2>
    </template>
        ...
        <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 justify-between mt-5">
          ...
          <div class="flex flex-col space-y-2">
              <p class="text-sm text-gray-700 leading-5 flex space-x-1">
                  <span>Showing</span>
                  <span class="font-medium">{{ vendCriteriaBindings.meta.from ?? 0 }}</span>
                  <span>to</span>
                  <span class="font-medium">{{ vendCriteriaBindings.meta.to ?? 0 }}</span>
                  <span>of</span>
                  <span class="font-medium">{{ vendCriteriaBindings.meta.total }}</span>
                  <span>results</span>
              </p>
              <MultiSelect
      ...
      <div class="mt-6 flex flex-col">
       <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="shadow-sm ring-1 ring-black ring-opacity-5 overflow-scroll">
            <table class="min-w-full border-separate" style="border-spacing: 0">
                <thead class="bg-gray-100">
                  ...
                  <tbody class="bg-white">
                    <tr v-for="(vendCriteriaBinding, vendCriteriaBindingIndex) in vendCriteriaBindings.data" :key="vendCriteriaBinding.id" class="divide-x divide-y-2 divide-gray-300 odd:bg-white even:bg-gray-100">
                      <TableData :currentIndex="vendCriteriaBindingIndex" :totalLength="vendCriteriaBindings.length" inputClass="text-center">
                        {{ vendCriteriaBindings.meta.from + vendCriteriaBindingIndex }}
                      </TableData>
                      <TableData :currentIndex="vendCriteriaBindingIndex" :totalLength="vendCriteriaBindings.length" inputClass="text-center">
                        {{ vendCriteriaBinding.code }}
                      </TableData>
                      <TableData :currentIndex="vendCriteriaBindingIndex" :totalLength="vendCriteriaBindings.length" inputClass="text-left">
                        {{ vendCriteriaBinding.full_name }}
                      </TableData>
                      <TableData :currentIndex="vendCriteriaBindingIndex" :totalLength="vendCriteriaBindings.length" inputClass="text-center">
                        <div class="flex justify-center space-x-1">
                          <Button
                            type="button" class="bg-gray-300 hover:bg-gray-400 px-3 py-2 text-xs text-gray-800 flex space-x-1"
                            @click="onEditClicked(vendCriteriaBinding)"
                          >
                          ...
                          <Button
                            ...
                            @click="onDeleteClicked(vendCriteriaBinding)"
                          >
                ...
                <tr v-if="!vendCriteriaBindings.data.length">
            ...
            <Paginator v-if="vendCriteriaBindings.data.length" :links="vendCriteriaBindings.links" :meta="vendCriteriaBindings.meta"></Paginator>
          </div>
      </div>
    </div>
  </div>
  <!-- <Form
      v-if="showModal"
      :vendCriteriaBinding="vendCriteriaBinding"
      :type="type"
      :showModal="showModal"
...
function onCreateClicked() {
  type.value = 'create'
  vendCriteriaBinding.value = null
  showModal.value = true
}

function onDeleteClicked(vendCriteriaBinding) {
  const approval = confirm('Are you sure to delete ' + vendCriteriaBinding.name + '?');
  if (!approval) {
      return;
  }
...
function onEditClicked(telcoValue) {
  type.value = 'update'
  vendCriteriaBinding.value = telcoValue
  showModal.value = true
}
