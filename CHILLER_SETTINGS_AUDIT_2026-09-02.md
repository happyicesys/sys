...
| Machine index rows | On Vend/Index and Vend/CustomerIndex (Operation Dashboard) a chiller row is rendered **cell-for-cell under the same column headers** by `SmartChillerRowCells` (multi-root: one `<TableData>` per header cell). Columns with no meaning for a chiller (temperature, modem/SIM, APK config, mark1 sales, payment device, contract terms) are left empty; applicable columns keep the vending cell's format — Machine ID (code, badge, serial, CityBox name, mirror mapping, site, postcode, Open Door/Pull), machine telemetry column (session state, last back online, last door-open, last poll), Inventory Status (channel list in the same `#code, remaining, qty/cap` grid → chiller layer overview; Value / Full Load / Stock Qty from their prices), Error (failed poll), Stock (qty/capacity % and SKUs-in-stock %), Last/Upcoming Job, Refilling Routes, Begin Dt, Machine Status (CityBox online/offline + ops status), Operator (+ model + Machine badge). Vending/freezer rows untouched. Both selects gained the three `vends.citybox_*` columns (plain columns, no join). The chiller layer overview accepts `vend_id` (index rows alias the PK). | `resources/js/Components/SmartChillerRowCells.vue`, `Vend/Index.vue`, `Vend/CustomerIndex.vue`, `Vend/SmartChillerChannelOverview.vue`, `VendController::index/indexCustomer`, `VendResource` |
...
| Channels from the planogram (2026-09-03) | Their `device_product` never reports a product the machine has never held (unit 10002: 2,178 polls, always the 3 drinks; `shipping_product` lists 7). Channels were built from the live call, so snacks on layers 2–5 never became `vend_channels` rows and never reached an ops job. `ChannelFrameAdapter` now emits one channel per planogram entry (qty 0, par-config price when unseen); `assignCodes` carries prices. | `ChannelFrameAdapter`, `ChillerPlanogram`, `tests/Feature/CityboxChannelsTest.php` |
| 3-digit channel codes (2026-09-03) | A layer can hold more than nine SKUs, so chiller channel codes become `<layer><position 2 digits>` (101…699). `ChillerPlanogram` owns the scheme (`channelCode`, `layerOf`, `isChillerCode`); `SyncVendChannels` accepts that range only for chillers; the planogram endpoint derives the layer from the hundreds digit; migration remaps existing rows. Index-row grouping keys on the first digit, unchanged. | `ChillerPlanogram`, `SyncVendChannels`, `CityboxVendActionController`, migration `2026_09_03_100000` |
