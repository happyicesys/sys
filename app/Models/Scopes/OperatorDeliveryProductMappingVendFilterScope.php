...
class OperatorDeliveryProductMappingVendFilterScope implements Scope
{
    /**
    ...
    public function apply(Builder $builder, Model $model)
    {
      if(auth()->check()) {
        ...
        if($operatorId) {
          $builder->whereHas('vend.operator', function($query) use ($operatorId) {
        ...
        if($vendIds) {
            ...
            if($customerIDs) {
              $builder->whereHas('vend.customer', function($query) use ($customerIDs) {
