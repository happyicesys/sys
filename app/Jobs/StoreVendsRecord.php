...
class StoreVendsRecord implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $faultStrict = DispenseVerdict::sqlFaultStrict('vt.vend_channel_error_id', 'vce.code');
        ...
        $itemsSub = DB::table('vend_transaction_items as vti')
            ->select(
                ...
                DB::raw('COUNT(CASE WHEN NOT ('.$itemSale.') THEN 1 ELSE NULL END) as item_error_count'),
                DB::raw('COUNT(CASE WHEN '.$itemSale.' THEN 1 ELSE NULL END) as success_item_count'),
        ...
        $query = DB::table('vend_transactions as vt')
            ...
            ->select(
                ...
                DB::raw('COALESCE(SUM(CASE
                    ...
                    WHEN '.$saleById.' THEN vt.revenue
                    ELSE 0 END), 0) as revenue'),

                DB::raw('COALESCE(SUM(CASE
                    WHEN vt.is_multiple = true THEN vt.revenue - COALESCE(vti.total_cost, 0)
                    WHEN '.$saleById.' THEN vt.gross_profit
                    ELSE 0 END), 0) as gross_profit'),

                DB::raw('SUM(CASE
                    WHEN vt.is_multiple = false AND '.$faultStrict.' THEN vt.amount
                    ELSE 0 END) as failure_amount'),

                DB::raw('SUM(CASE
                    WHEN vt.is_multiple = false AND '.$faultStrict.' THEN COALESCE(vt.qty, 1)
                    ELSE 0 END) as failure_count'),

                DB::raw('SUM(CASE
                    WHEN dpo.id IS NOT NULL AND (vt.is_multiple = true OR '.$saleById.') THEN vt.amount
                    ELSE 0 END) as online_success_amount'),

                DB::raw('SUM(CASE
                    WHEN dpo.id IS NOT NULL AND (vt.is_multiple = true OR '.$saleById.') THEN COALESCE(vt.qty, 1)
                    ELSE 0 END) as online_success_count'),

                DB::raw('SUM(CASE
                    WHEN dpo.id IS NOT NULL AND vt.is_multiple = false AND '.$faultStrict.' THEN vt.amount
                    ELSE 0 END) as online_failure_amount'),

                DB::raw('SUM(CASE
                    WHEN dpo.id IS NOT NULL AND vt.is_multiple = false AND '.$faultStrict.' THEN COALESCE(vt.qty, 1)
