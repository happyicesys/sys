...
class HandleFailedVendTransaction implements ShouldQueue
{
    ...
    public function handle(): void
    {
        if ($this->vendTransaction->paymentGatewayLog()->exists()) {
            ...
            // Same shared gate the settlement resolver demotes on — the two
            // must agree (see PaymentGateway::supportsApiRefund). Omise is the
            // only API-refundable gateway today; when Fiuu/Midtrans refunds
            // land, add a dispatch branch here and widen supportsApiRefund in
            // the same change.
            if ($paymentGateway?->supportsApiRefund()) {
