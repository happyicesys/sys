        ...
        if($vend) {
            ...
            $vend->update([
                'vend_transaction_totals_json' => [
                    ...
                    'vend_records_amount_average_day' => ((int)$vend->lifetimeVendRecords->sum('total_amount') + $todayAmount)/ (int)($vend->begin_date ? Carbon::parse($vend->begin_date)->diffInDays(Carbon::parse($vend->termination_date ?: Carbon::now())) : 1),
        ...
        if($customer) {
            ...
            $customer->update([
                'totals_json' => [
                    ...
                    'vend_records_amount_average_day' => ((int)$customer->lifetimeVendRecords->sum('total_amount') + $todayAmount)/ (int)($customer->begin_date ? Carbon::parse($customer->begin_date)->diffInDays(Carbon::parse($customer->termination_date ?: Carbon::now())) : 1),
