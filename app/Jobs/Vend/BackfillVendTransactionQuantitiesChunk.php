...
class BackfillVendTransactionQuantitiesChunk implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        DB::statement(
            ...
            JOIN (
                SELECT vend_transaction_id,
                       ...
                       SUM(CASE WHEN CAST(vend_channel_error_code AS SIGNED) IN ('.DispenseVerdict::dispensedList().') THEN 1 ELSE 0 END) AS success_items,
                       SUM(CASE WHEN CAST(vend_channel_error_code AS SIGNED) IN ('.DispenseVerdict::droppedList().') THEN 1 ELSE 0 END) AS dispensed_items
        ...
        DB::statement(
            ...
            SET vt.qty = 1,
                vt.success_qty = CASE WHEN CAST(vce.code AS SIGNED) IN ('.DispenseVerdict::dispensedList().') THEN 1 ELSE 0 END,
                vt.dispensed_qty = CASE WHEN CAST(vce.code AS SIGNED) IN ('.DispenseVerdict::droppedList().') THEN 1 ELSE 0 END,
                vt.updated_at = NOW()
            WHERE vt.id BETWEEN ? AND ?
              AND NOT EXISTS (
                  SELECT 1
