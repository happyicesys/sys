...
class VendDataService
{
    ...
    public function processVendData($originalInput, $processedInput, $ipAddress, $connectionType)
    {
        ...
        // Staged ack-format restore (ACK_FIX_PLAN.md, apk/mark1-apk). The APK
        // parses "<fid>,<len>,<base64>" — response()->json() wraps that in literal
        // quotes, which has broken every HTTP ack fleet-wide since 2025-07-05 and
        // drives the duplicate-TRADE re-uploads. Vends in raw_ack_vends get the
        // raw string back (the pre-2025 behaviour, still what MQTT publishes);
        // everyone else keeps the JSON-wrapped ack until the rollout widens.
        // The is_string guard keeps the no-'f' edge case ($response === true) on
        // the JSON path, identical for both groups.
        $rawAckVends = config('app.raw_ack_vends', []);
        $rawAck = is_string($response)
          && (in_array('*', $rawAckVends, true)
