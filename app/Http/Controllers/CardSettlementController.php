...
class CardSettlementController extends Controller
{
    ...
    public function show(Request $request, $id, CardSettlementOrphanSales $orphans)
    {
        ...
        $suspectBindings = app(TerminalMoveSuggestions::class)->suspects($report);

        // Terminals in this report with no binding — the one-time setup the
        // user must do before those rows can ever match.
        $unboundTerminals = app(TerminalMoveSuggestions::class)->unbound($report);
    ...
    public function fixBindings(Request $request, $id, CardTerminalBindingService $bindings)
    {
        ...
        $suspects = app(TerminalMoveSuggestions::class)->suspects($report);
        ...
        [$moved, $skipped] = app(TerminalMoveSuggestions::class)->applySuspects($suspects);
    ...
    public function bindUnbound(Request $request, $id, CardTerminalBindingService $bindings)
    {
        ...
        $chosen = collect($validated['terminal_ids'])->map(fn ($t) => (string) $t)->all();
        $targets = app(TerminalMoveSuggestions::class)->unbound($report)
            ->whereIn('terminal_id', $chosen)
            ->filter(fn ($t) => $t['suggested_vend_code'] !== null);

        [$bound, $skipped] = app(TerminalMoveSuggestions::class)->applyUnbound($report, $targets);
