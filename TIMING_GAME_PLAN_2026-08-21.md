...
## 2b. Visual direction — **B · Happy Ice Pop shortlisted (Brian, 2026-08-22)**
...
how long they stay visible), result as a **signed delta** ("+0.327 s over"), a
start-beep and stop-click, and a best-record readout.

**B · Happy Ice Pop** (chosen over A Arcade Night / C Broadcast):
- Cream stage (`#FFF7E6`, soft pink/blue radial tints) over the dimmed idle
  screen; Baloo 2 for headings/labels, Fredoka for the big numerals; brand red
  `#D92D20` title, yellow `#ffda00` START/STOP pill with a 12dp `#E0B800`
  "3D" drop, green `#009644` for the win.
- **The clock is a cone.** Ten pastel scoops = 10 s; one scoop stacks per
  second for `hideAfterMs` (default 3 s), then stacking stops and the customer
  counts the rest. Big one-decimal counter while visible ("2.4"), frozen at
  "3.0" once hidden; full three-decimal reveal on stop ("10.004").
- Win: all ten scoops + a cherry, sprinkles, `PERFECT!` stamp, prize card
  (name, slot, $0.00, DROPPING) → "Collect it from the tray below".
  Lose: an 11th scoop topples off the stack (over-shoot), `SO CLOSE` /
  `NOT BAD` / `WAY OFF`, "+0.327 s over · you needed ±0.050 s", ORDER NOW
  upsell + "next free try in mm:ss".
- Audio: short beep on START, click on STOP, chime on win (APK `SoundPool`;
  assets ≤ 50 KB each, no `getIdentifier()`).
- "Best today / plays today" = device-local prefs
  (`timingGame.bestMsDate/bestMs/playsCount`), reset at local midnight.
- Freezer: same cone on a cream stage; offer card on Confirm & pay, prize line
  with a `PRIZE` badge after a win.

**Screen sizes (render targets).** Big board ships on **7" and 11" portrait**
panels; the APK is authored at 720dp width (`activity_main2.xml`, `dimens.xml`).
Design is dp-anchored and fluid: content column `max-width 620dp`, centred, so
the 11" (assumed ~800×1280dp — **confirm with `adb shell wm size && wm density`
on an 11" unit**) gets the same dp sizes with more margin and is physically
~1.5× larger. If the 11" reports a much larger dp width (e.g. ≥ 1000dp), add a
`values-w1000dp/dimens.xml` bumping the game's type/scoop sizes ~1.2× — no
layout change. Touch floors: STOP pill 132dp tall, full content width; all
secondary targets ≥ 56dp (gloved/child use). Freezer **21.5" landscape
1920×1080** follows `KioskDimens` (scale 1.0, primary targets 80dp+): cone
left (1.25×), copy + 150dp STOP pill right — thumb zone for a standing adult.
